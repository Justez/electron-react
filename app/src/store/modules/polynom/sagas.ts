...
export function* calculate() { // add timeout 
    try {
        ...
        if (!(polynom.calcCount)) {
            ...
            const height = ~~(document.getElementsByTagName('body')[0].clientHeight / 100 * 80);
        ...
        for (let x = 0; x < canvas.width; x++) {
            for (let y = 0; y < canvas.height; y++) {
                ...
                if (!colorIntensity) {
                    ctx.fillStyle = settings.contrastFill ? '#fff' : '#000';
                } else {
                    ctx.fillStyle = `hsl(${settings.colorBase}, ${settings.colorDensity}%, ${colorIntensity}%)`;
                }
...
export default function* saga() {
    yield all([
        takeLatest(polynomActions.paint, calculate),
    ]);
}